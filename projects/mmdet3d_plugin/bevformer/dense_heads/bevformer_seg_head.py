...
class BEVFormerHead_seg(BaseDenseHead, BBoxTestMixin):
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            # MapTR transformer.get_bev_features signature is:
            #   get_bev_features(mlvl_feats, lidar_feat, bev_queries, bev_h, bev_w, ...)
            # We don't have lidar features here, so pass lidar_feat=None explicitly.
            ...
            # MapTR transformer.forward signature is:
            #   forward(mlvl_feats, lidar_feat, bev_queries, object_query_embed, bev_h, bev_w, ...)
            outputs = self.transformer(
                mlvl_feats,
                None,  # lidar_feat placeholder
                bev_queries,
                object_query_embeds,
                self.bev_h,
                self.bev_w,
        ...
        # MapTR.transformer.forward returns: (bev_embed, depth, inter_states, init_reference_out, inter_references_out)

import torch
# import mmcv  # keep if used elsewhere
# import tempfile
...
class IntersectionOverUnion(Metric):
    """Computes intersection-over-union (per-class scores)."""
    ...
    def __init__(
        ...
        # NOTE: do NOT pass compute_on_step to super() — older/newer torchmetrics differ.
        super().__init__()
        ...
        # states for accumulation
        self.add_state('true_positive', default=torch.zeros(
            n_classes, dtype=torch.float64), dist_reduce_fx='sum')
        ...
        # instantiate a MulticlassStatScores metric to compute tp/fp/tn/fn/support
        # use average=None -> per-class stats
        # move to cpu by default; .to(device) will be called implicitly when used
        self._stat_metric = MulticlassStatScores(num_classes=self.n_classes, average=None)

    def update(self, prediction: torch.Tensor, target: torch.Tensor):
        """
        prediction: tensor of shape (N, H, W) or (batch, ...) with integer class preds
        target: same shape as prediction with integer labels
        """
        # ensure long/int types
        preds = prediction.long().flatten()
        targs = target.long().flatten()

        # call the MulticlassStatScores callable
        stats = self._stat_metric(preds, targs)  # returns a tensor of shape (num_classes, 5) or (5, num_classes)
        # be defensive about shape:
        if stats.ndim == 2 and stats.shape[1] == 5:
            # shape: (num_classes, 5) -> columns are (tp, fp, tn, fn, support)
            tp = stats[:, 0].to(torch.float64)
            fp = stats[:, 1].to(torch.float64)
            fn = stats[:, 3].to(torch.float64)
            sup = stats[:, 4].to(torch.float64)
        elif stats.ndim == 2 and stats.shape[0] == 5:
            # shape: (5, num_classes)
            tp = stats[0, :].to(torch.float64)
            fp = stats[1, :].to(torch.float64)
            fn = stats[3, :].to(torch.float64)
            ...
            tp = torch.zeros(self.n_classes, dtype=torch.float64, device=preds.device)
            fp = torch.zeros_like(tp)
            fn = torch.zeros_like(tp)
            sup = torch.zeros_like(tp)
    ...
    def compute(self):
        ...
        for class_idx in range(self.n_classes):
            ...
            # absent in both pred and target
            if sup + tp + fp == 0:
                scores[class_idx] = float(self.absent_score)
                continue

            denominator = tp + fp + fn
            score = (tp.to(torch.float32) / (denominator + 1e-12))
            scores[class_idx] = score

        # Remove ignored class index from the scores.

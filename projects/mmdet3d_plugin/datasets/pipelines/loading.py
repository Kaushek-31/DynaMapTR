...
@PIPELINES.register_module()
class LoadObjectSegMask:
    """Load object segmentation mask from data dict."""
    
    def __init__(self, to_float32=False):
        self.to_float32 = to_float32
    
    def __call__(self, results):
        """Load object seg mask.
        
        Args:
            results (dict): Result dict from loading pipeline
        
        Returns:
            dict: Results with 'object_seg_mask' added
        """
        if 'object_seg_mask' in results:
            object_seg_mask = results['object_seg_mask']
            
            if self.to_float32:
                object_seg_mask = object_seg_mask.astype(np.float32)
            
            results['object_seg_mask'] = object_seg_mask

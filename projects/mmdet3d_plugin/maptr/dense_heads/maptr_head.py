    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        if self.bev_embedding is not None:
            ...
            if os.environ.get('MAPTR_DEBUG', '0') == '1':
                print(f"DEBUG MapTRHead.forward: pc_range={self.pc_range}, real_h={self.real_h}, real_w={self.real_w}")
                print(f"DEBUG bev shapes: bev_mask={bev_mask.shape}, bev_pos={None if bev_pos is None else bev_pos.shape}, bev_queries_len={bev_queries.shape[0]}")
                # quick normalization round-trip check
                try:
                    sample = torch.tensor([[self.pc_range[0]+1.0, self.pc_range[1]+1.0]], dtype=torch.float32, device=bev_queries.device)
                    norm = normalize_2d_pts(sample, torch.tensor(self.pc_range, dtype=sample.dtype, device=sample.device))
                    den = denormalize_2d_pts(norm, torch.tensor(self.pc_range, dtype=sample.dtype, device=sample.device))
                    print("DEBUG normalize roundtrip maxdiff:", (sample - den).abs().max().item())
                except Exception as e:
                    print("DEBUG MapTRHead.forward normalization check failed:", e)
        else:
            bev_queries = None
            bev_mask = None
            bev_pos = None

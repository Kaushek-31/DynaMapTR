...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        # Adapter (1x1 conv) to align BEVFormer encoder channels the first
        # time we detect a mismatch with MapTR's embed_dims.
        # self.bevformer_adapter = None
    ...
    def attn_bev_encode(
        ...
        # # DEBUG: ensure bev token count and inspect CAN-bus when enabled
        # if os.environ.get('MAPTR_DEBUG', '0') == '1':
        #     print(f"DEBUG attn_bev_encode: bs={bs}, bev_h={bev_h}, bev_w={bev_w}, bev_queries_tokens={bev_queries.shape[0]}")
        #     try:
        #         sample_can = kwargs['img_metas'][0].get('can_bus') if isinstance(kwargs['img_metas'][0], dict) else kwargs['img_metas'][0]
        #     except Exception:
        #         sample_can = kwargs['img_metas'][0]
        #     print("DEBUG sample can_bus:", sample_can)

        # assert bev_queries.shape[0] == bev_h * bev_w, f"Unexpected bev token count {bev_queries.shape[0]} vs bev_h*bev_w {bev_h*bev_w}"
        ...
        if prev_bev is not None:
            # print(f"DEBUG prev_bev type: {type(prev_bev)}")
            
            # Handle both dict and tensor formats
            if isinstance(prev_bev, dict):
                # print(f"DEBUG prev_bev keys: {prev_bev.keys()}")
                prev_bev = prev_bev['bev']
            # print(f"DEBUG prev_bev shape after extraction: {prev_bev.shape}")
            # Ensure correct shape format
        ...
        if isinstance(ret_dict, dict):
            bev_embed = ret_dict.get('bev', None)
            depth = ret_dict.get('depth', None)
        else:
            bev_embed = ret_dict
            depth = None

        if bev_embed is None:
            raise RuntimeError('BEVFormer encoder returned an empty bev feature map.')
    ...
    def forward_decoder(self,
                    ...
                    **kwargs):
        """
        Forward pass for decoder only (after masking).
        
        Args:
            bev_embed: Masked BEV features (H*W, B, C)
            mlvl_feats: Multi-level image features
            object_query_embed: Query embeddings for decoder
            bev_h, bev_w: BEV dimensions
        
        Returns:
            inter_states, init_reference_out, inter_references_out
        """
        bs = mlvl_feats[0].size(0)
        ...
        # Ensure bev_embed is in (H*W, B, C) format
        if bev_embed.shape[1] != bs:
            bev_embed = bev_embed.permute(1, 0, 2)

        # Format multi-level features
        ...
        inter_states, inter_references = self.decoder(
            query=query,
            key=None,
            value=bev_embed,  # ← Masked BEV features!
            query_pos=query_pos,

...
def single_gpu_test(model,
    ...
    if map_enable:
        ...
        # ============================================================
        # ADD: Object segmentation metric (11 classes)
        # ============================================================
        num_object_class = 11
        object_seg_iou_val = IntersectionOverUnion(num_object_class)
        object_seg_iou_val = object_seg_iou_val.cuda()
        object_enable = False
    ...
    for i, data in enumerate(data_loader):
        ...
        # ============================================================
        # MODIFIED: Separate evaluation for map and object segmentation
        # ============================================================
        if map_enable and result[0]['seg_preds'] is not None:
            ...
            # Check number of classes and route to appropriate metric
            if num_cls == 4:
                # Map segmentation (divider, ped_crossing, boundary + background)
                target_semantic_indices = data['semantic_indices'][0].cuda()
                semantic_map_iou_val(pred_semantic_indices, target_semantic_indices)
                logger.info(f"Evaluating map segmentation (4 classes)")
            
            elif num_cls == 11:
                # Object segmentation (10 object classes + background)
                object_enable = True
                if 'gt_object_seg_mask' in data:
                    target_object_indices = data['gt_object_seg_mask'][0].cuda()
                    # Remove channel dimension if present: (1, H, W) → (H, W)
                    if target_object_indices.dim() == 3 and target_object_indices.shape[0] == 1:
                        target_object_indices = target_object_indices.squeeze(0)
                    object_seg_iou_val(pred_semantic_indices, target_object_indices)
                    logger.info(f"Evaluating object segmentation (11 classes)")
                else:
                    logger.warning("gt_object_seg_mask not found in data, skipping object evaluation")
            
            else:
                logger.warning(f"Unexpected number of segmentation classes: {num_cls}. Skipping evaluation.")

        for _ in range(batch_size):
            prog_bar.update()

    # ============================================================
    # MODIFIED: Print results for both map and object segmentation
    # ============================================================
    if map_enable:
        ...
        with open('map_segmentation_result.json', 'a') as f:
            f.write(json.dumps(str(seg_dict)) + '\n')
        
        # Object segmentation results
        if object_enable:
            obj_scores = object_seg_iou_val.compute()
            obj_mIoU = sum(obj_scores[1:]) / (len(obj_scores) - 1)
            
            obj_classes = ['Car', 'Truck', 'Construction', 'Bus', 'Trailer', 
                          'Barrier', 'Motorcycle', 'Bicycle', 'Pedestrian', 'Traffic Cone']
            
            tb_obj = pt.PrettyTable()
            tb_obj.field_names = ['Validation num'] + obj_classes + ['mIoU']
            row = [len(dataset)] + [round(s.cpu().numpy().item(), 4) for s in obj_scores[1:]] + [round(obj_mIoU.cpu().numpy().item(), 4)]
            tb_obj.add_row(row)
            ...
            obj_dict = dict(
                Validation_num=len(dataset),
                **{cls: round(obj_scores[i+1].cpu().numpy().item(), 4) for i, cls in enumerate(obj_classes)},
                mIoU=round(obj_mIoU.cpu().numpy().item(), 4)
            )
            with open('object_segmentation_result.json', 'a') as f:
                f.write(json.dumps(str(obj_dict)) + '\n')
        ...
        if map_enable and result[0]['seg_preds'] is not None:
            ...
            if num_cls == 4:
                ...
                if rank == 0 and i % 50 == 0:
                    logger.info(f"Evaluating map segmentation (4 classes) - batch {i}")
                ...
                if 'gt_object_seg_mask' in data:
                    ...
                    object_seg_iou_val(pred_semantic_indices, target_object_indices)
                    if rank == 0 and i % 50 == 0:
                        logger.info(f"Evaluating object segmentation (11 classes) - batch {i}")
                else:
                    if rank == 0 and i == 0:
                        logger.warning("gt_object_seg_mask not found in data, skipping object evaluation")
